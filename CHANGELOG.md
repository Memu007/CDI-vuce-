...
- **feat (sin costo):** búsqueda sin IA ni internet: código exacto, mayúsculas/tildes/plurales/puntuación y sinónimos comerciales frecuentes (notebook, mesa, remera, motor, tornillo, etc.). Gemini queda como último fallback opt-in con `NCM_GEMINI_FALLBACK=true`; está apagado por defecto.
- **seguridad (MARIA):** las sugerencias de ARCA son NCM de 8 cifras: al elegir una se exige elegir después una posición SIM completa de 11 cifras + DC desde VUCE antes de continuar o generar. Las confirmaciones aprenden por cliente y proveedor sin reutilizar automáticamente una SIM/DC anterior.
- **test:** 100 pruebas focalizadas en verde, incluidas 20 rutas de `VUCE_CI_ENABLED=true` con alternativas SIM y fallback sin datos falsos.
