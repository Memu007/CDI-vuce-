...
- **Asistente NCM sin costo — catálogo ARCA (2026-07-12):** `proyecto_maria/data/ncm_arca.csv.gz` contiene 10.226 NCM de 8 dígitos generadas desde el ZIP oficial de ARCA; `ncm_arca_metadata.json` conserva URL, fecha publicada por ARCA (`2026-07-12`) y SHA-256. El endpoint `POST /api/ncm/sugerir` ordena: memoria confirmada de cliente, catálogo del proveedor, historial del despachante, catálogo ARCA y sólo al final Gemini si `NCM_GEMINI_FALLBACK=true` (default `false`). Máximo 5 opciones. La búsqueda es local (sin internet/IA), tolera tildes, plurales, puntuación, código exacto y sinónimos. Para actualizar el catálogo: descargar el ZIP oficial y ejecutar `PYTHONPATH=. python scripts/import_arancel_nomenclador.py --archive /ruta/arancel.zip --updated-at AAAA-MM-DD`; no inventar ni editar códigos/descripciones a mano. La UI identifica cada propuesta como Historial, ARCA o IA orientativa y muestra la fecha de ARCA.
