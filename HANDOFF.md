...
> Última actualización: 2026-07-26 · Revisión de deploy: scripts de Cloud Run arreglados, preflight de variables, secretos tachados de docs. Ver `docs/deployment/ESTADO_DEPLOY.md`.
...
- **Suite de tests destrabada (2026-07-26):** `pytest` completo corre por primera vez — **658 passed, 102 skipped, ~42s**. Causa raíz del cuelgue: `TestClient` sin `with` abre un event loop nuevo por request y el `StaticPool` del `conftest` reusaba la misma conexión de aiosqlite entre loops; al cerrarla desde un loop muerto quedaba colgada. Ahora `NullPool` + `pytest_sessionfinish` que apaga conexiones huérfanas con el centinela de aiosqlite. **CI corre la suite entera**, con 10 fallas viejas deseleccionadas una por una en `ci.yml` (6 de `test_pilar_b_quotes`, 3 de botones NCM en HTML/JS, 1 de CSRF con Bearer) — sacarlas de esa lista a medida que se arreglen. **Hallazgo:** había 23 tests de "sin sesión → 401" que nunca podían pasar porque la app autentica sola bajo pytest; la fixture autouse `_apagar_bypass_en_tests_de_auth` los habilita por nombre.

- **Supuestos a la vista (2026-07-26):** `supuestos_declarados()` (en `maria_generator.py`) lista antes de generar todo campo que el sistema declaró sin preguntar — gastos a FOB calculados u omitidos, procedencia igualada al origen con el número de ítem, `CARTUSO=3`, `IVAADICIONAL1=IVAAD1` y unidad `07` por defecto. `/generate_maria` los devuelve en `supuestos` y los suma a `warnings`, que el frontend ya renderiza. No bloquean. **Regla de fondo:** ningún campo se inventa y ninguno cambia sin evidencia de un archivo real; lo que no se sabe, se declara como siempre y se avisa. Decisión completa, con el ataque adversarial contra ella misma, en `docs/features/DECISION_SUPUESTOS_MARIA.md`. **El punto discutible está marcado ahí:** el cambio de gastos a FOB en EXW alteró la salida basándose en parte en fuentes web que no se pudieron abrir; se revierte en un solo lugar si el despachante lo desaprueba.

- **Mapa de origen de datos (2026-07-26):** `docs/features/ORIGEN_DE_LOS_DATOS_MARIA.md` clasifica los 71 campos del MARIA.TXT por origen (factura / fijo / se carga una vez / lo decide el despachante / calculado), cruzando los dos archivos reales, el manual de PreDespacho y lo que extrae `pdf_extractor.py`. **Hallazgo pendiente de arreglo:** `CARTPAYORI` (origen) y `CARTPAYPRC` (procedencia) son campos distintos y en `maria_golden_anon.TXT` difieren (200 vs 222); el generador los iguala por defecto (`maria_generator.py`, línea del `CARTPAYPRC`), así que cuando la mercadería se embarca desde un país distinto al de origen el TXT sale mal sin que ninguna validación avise.

- **Gastos a FOB por condición de venta (2026-07-26):** se emitía siempre `GTOS-POS-FOB` con flete + seguro; en una operación EXW ese es el campo equivocado con el número equivocado. Ahora: EXW → `GTOS-ANT-FOB`, grupos C/D (`_INCOTERMS_GRUPO_C_D`) → `GTOS-POS-FOB`. El importe es la diferencia entre la condición pactada y el FOB, no flete + seguro: se acepta explícito vía `gastos_fob` (parámetro del generador y campo de `/generate_maria`); sin él se calcula flete + seguro sólo en C/D y fuera de ahí **no se declara nada** en vez de inventar. `tests/test_maria_gastos_fob.py`. **Falta la pantalla** para cargar el importe, y las fuentes son secundarias (AFIP/CDA/Infoleg no se pudieron abrir desde el entorno de la IA): conviene que el despachante lo confirme.

- **Sub-ítems en el MARIA.TXT (2026-07-26):** `generate_maria_txt` acepta `subitems` dentro de cada ítem (cantidad, valor unitario y sufijo propios) y emite un `[SBT]` por sub-ítem con `MSBTFOB`, `QSBTUNTDCL`, `MSBTUNITAR` y `QSBTUNTEST`, marcando `CARTSBITEM=S`. Verificado contra un archivo real: los bloques salen idénticos (`tests/test_maria_subitems.py`). Si las cantidades o los valores de los sub-ítems no suman exactamente lo del ítem, corta con error en vez de generar un TXT que no cuadra. Sin `subitems` el comportamiento es el de siempre. **Falta la pantalla:** el despachante todavía no tiene dónde partir un ítem desde la app.

- **Año del `IEXT` arreglado (2026-07-26):** `maria_generator` armaba `IEXT=00272-01/25` con el año fijo en el código, así que toda declaración de 2026 en adelante salía con el año equivocado sin que ninguna validación se quejara. Ahora sale de `fecha_emision`, con el año actual como respaldo. Detectado comparando contra un MARIA.TXT real aportado por el dueño (guardado anonimizado en `tests/fixtures/maria_golden_subitems_anon.TXT`). Cubierto por `tests/test_maria_iext_anio.py`. **Pendiente de definición del despachante:** código de proveedor entre paréntesis en `LDDTNOMFOD`, sub-ítems múltiples en `[SBT]` (hoy se emite uno solo y sin montos), `GTOS-ANT-FOB` vs `GTOS-POS-FOB` según incoterm, `CARTUSO` fijo en 3, y el `[CPL]` de banco. Tabla completa en `docs/deployment/PLAN_DEPLOY_ADVERSARIAL.md`, ronda G4.

- **Descargas aisladas por usuario (2026-07-26):** `/download/{archivo}` pedía sesión pero no verificaba dueño. Como el nombre del MARIA.TXT sale del número de factura (`MARIA_FAC_0001-00012345.TXT`), cualquier usuario registrado podía bajarse la declaración de otro despachante probando números — y de paso cualquier archivo suelto en `DATA_DIR` (`ncm_historial_<usuario>.json`, backups). Ahora los descargables van a `DATA_DIR/descargas/<hash del usuario>/` vía `_user_downloads_dir()` y `/download` resuelve sólo ahí. Alcanza a `/generate_maria`, `/generate_maria_export` y `create_maria_excel` (que además dejó de ignorar `CDI_DATA_DIR`). Cubierto por `tests/test_download_aislamiento.py`. Encontrado con `./scripts/testing/ataque_aislamiento.sh` (ronda G3): primera corrida 1 fuga, segunda 22 bloqueados / 0 fugas.

- **Deploy revisado (2026-07-26):** informe completo en `docs/deployment/ESTADO_DEPLOY.md`. Resumen: la app arranca bien con `ENVIRONMENT=production` (verificado: `/health` ok, `/docs` cerrado, usuarios demo no se crean, cookies `Secure`, cabeceras de seguridad completas). Se arreglaron los dos scripts de Cloud Run, que no pasaban `JWT_SECRET_KEY` / `ALLOWED_ORIGINS` / `DATABASE_URL` — `scripts/deployment/deploy-cloud-run.sh` ni siquiera seteaba `ENVIRONMENT=production`. Nuevo `scripts/deployment/preflight_env.sh` valida las variables antes de buildear. **Destino decidido: Railway** (2026-07-26). Se borraron `firebase.json` (apuntaba a una función inexistente) y `deploy_cloudrun.sh` de la raíz (deployaba un servicio distinto al del otro script); queda `scripts/deployment/deploy-cloud-run.sh` como alternativa arreglada. Ronda G2 del plan adversarial corrida en modo `--sin-docker` contra Postgres real: **16 chequeos, 16 resistidos, 0 caídos** (evidencia en `PLAN_DEPLOY_ADVERSARIAL.md`). **Pendiente del humano:** rotar la `GEMINI_API_KEY` que quedó en el historial de git (estaba en `docs/audits/AUDITORIA_PRE_TESTING_5_USUARIOS.md`; escaneo completo de los 53 commits: es la única clave viva) y correr `./scripts/testing/ataque_imagen.sh` (modo completo) en una máquina con Docker y salida normal a internet — acá la política de red bloquea la descarga de imágenes base.
- **Suite completa colgada (2026-07-26):** `pytest` sobre todo el repo no termina. Cuelga en `tests/test_billing_autoservicio.py::test_change_password_wrong_current_returns_401`, `tests/test_prelaunch_block2.py::test_trial_vencido_bloquea_operaciones_con_402` y `tests/test_prelaunch_block3.py::test_past_due_rechaza_operacion_con_402`; al deseleccionar los tres aparece otro en el mismo `test_billing_autoservicio.py`, así que hoy no hay número confiable de "cuántas pasan". Todos del mismo palo (billing / trial / 402), probablemente una sola causa. El `--timeout` de `pytest.ini` no los mata porque usa `--timeout-method=thread`. Deseleccionando esos tres quedan además pruebas en rojo en `test_prelaunch_block1.py`, `test_prelaunch_block3.py`, `test_pilar_b_quotes.py` y `test_pdf_avg_functional.py`. CI no lo detecta porque corre solo 4 archivos elegidos a mano más el E2E. No frena el deploy, pero "las pruebas pasan" hoy vale solo para esos 4 archivos.
...
- **Flujo NCM → SIM sin falso error (2026-07-12):** seleccionar una sugerencia ARCA/historial de 8 dígitos inicia inmediatamente la consulta VUCE y muestra un estado de carga explicativo; no dispara toast de error ni asigna la NCM base. Con una sola SIM se puede confirmar; con varias, el selector arranca sin opción elegida y obliga al despachante a seleccionar una posición 11 + DC. Fallas de VUCE quedan visibles con Reintentar.
- **Asistente NCM — catálogo ARCA solo para código exacto/validación, no como clasificador (2026-07-13):** se detectó regresión sistémica: el buscador lexical (`ncm_catalog.search_term`) usaba coincidencia de palabras contra el nomenclador ARCA, que acumula contexto legal de capítulos/partidas — compartir una palabra no significa pertenecer al producto correcto (ej. "TABLE" → tableros de partículas; "botella de acero inoxidable" → acero en lingotes). Se sacó el buscador lexical como fuente de sugerencias comerciales. Prioridad actual de `POST /api/ncm/sugerir`: 1) memoria confirmada de cliente, 2) catálogo confirmado del proveedor, 3) historial liviano del despachante, 4) si la consulta es un código NCM exacto/parcial (dígitos), lookup directo en ARCA, 5) si no hay memoria ni código, Gemini (`GEMINI_API_KEY` + `NCM_GEMINI_FALLBACK` no en `false`, on por defecto si hay key) propone hasta 5 candidatos que se validan contra ARCA — cualquier código que Gemini invente y no exista en el catálogo se descarta (`_validate_ia_ncm_candidates`). Sin memoria, sin código exacto y sin Gemini disponible: `sugerencias: []` + `message: "Sin coincidencia segura. Agregá material, uso o tipo de producto."`. `ncm_arca.csv.gz` (10.226 NCM) y `ncm_catalog.search_term` siguen usándose para código exacto y para resolver la descripción oficial de un candidato validado. Frontend (`ncm.js`) muestra ese mensaje en el spotlight cuando `sugerencias` viene vacío con `message`. Tests: `tests/test_ncm_sugerir_catalog.py` (24 casos, incluye TABLE/acero inoxidable sin memoria → [], Gemini mockeado validado, código inventado descartado).
- **VUCE CI tras flag (2026-07-12):** `VUCE_CI_ENABLED` sigue en `false`. Hay 20 pruebas determinísticas que verifican el camino oficial, alternativas SIM y fallback cuando CI no responde; cache y rate limit existentes se preservan. Una sonda real contra `qa.ci.vuce.gob.ar` el 12/07 tuvo timeouts, por lo que **no** se activó en Railway: antes de habilitar el flag confirmar conectividad y una muestra real desde el entorno de deploy. Nunca se etiqueta fallback/fake como fuente oficial.
...
- **Inferencia masiva desde una fila (2026-07-12):** si los seleccionados contienen una única posición SIM completa, la barra masiva la precarga automáticamente (incluido DC); “Asignar SIM y unir” la copia a los ítems vacíos/incompletos y crea el grupo. La lectura toma primero los campos SIM/DC visibles (aunque el estado interno todavía no haya sincronizado) y después el item persistido. Dos posiciones completas diferentes requieren elección manual y no se unen silenciosamente.
- **Visual de grupos (2026-07-12):** las filas/chips agrupados usan azul suave, no amarillo; el amarillo queda reservado para estados pendientes o advertencias.
...
- **Tour de ingreso V3 (2026-07-13):** un único onboarding de 4 pasos reemplaza el tour de 7 slides y la tarjeta `welcomeCard` duplicada. Explica el contrato real: subir prepara datos; el despachante revisa carátula/pesos/origen y confirma SIM 11 + DC; con cliente activo, generar guarda operación e historial; SBT y validación en Kit MARIA siguen siendo obligatorios. CTA final enfoca “Seleccionar archivo”. Estado en `cdi_tour_v3`, con migración de completado/omitido desde v2 y reapertura desde Perfil.
...
- **Hotfix SBT con frontend (2026-07-12):** el dashboard v2 tiene campo "Sufijo de valor SBT" en pantalla de revisión (obligatorio, con ayuda "Consultar al despachante"). Backend valida trim, máximo 120 chars, rechaza `\r`/`\n` y caracteres de control. Error del backend se muestra junto al campo. Sin fallback VOWYNNS/VITTO. `buildMariaRequest()` en `finalize.js` envía `sbt_sufijo_valor`. 547 tests, 0 failures, cobertura 39.05%.
...
- **Datos de carátula fase 1 (2026-07-06):** pantalla Revisión ahora muestra "Domicilio del establecimiento" y "Fecha inicio de actividades" en sección Importador (se auto-pueblan desde la ficha del cliente al elegir importador, con hint verde). Campo nuevo "Gastos anteriores a FOB (USD)" en Costos adicionales (numérico, opcional). Sin cambios en DB. **Auditoría contra `CDI - INFORMACIÓN CARÁTULA E ITEMS.xlsx` (2026-07-12):** IC04 ya cubre CUIT, domicilio, inicio de actividades, Incoterm, proveedor/ID, fechas, flete y seguro; `gastos_anteriores_fob` se captura en la UI pero todavía no se serializa en el TXT. EC01, gastos Art. 736 y lugar Art. 736 siguen pendientes: el Excel no aporta las claves MARIA necesarias para implementarlos sin inventar datos.
...
- **Iteración C — Bug Fixes & UX (Cerrada):**
  ...
  - **Tour V3 (vigente):** recorrido reducido a 4 pasos, sin promesas falsas y sin la tarjeta de bienvenida duplicada. Conserva el track animado, suma navegación accesible y termina en la carga de factura.
...
- **Antes de deployar (2026-07-26)**: rotar la `GEMINI_API_KEY` que quedó en el historial de git y decidir un único destino de deploy (hoy conviven Railway, dos servicios de Cloud Run y un `firebase.json` muerto que apunta a una función inexistente). Detalle y pasos en `docs/deployment/ESTADO_DEPLOY.md`.
- ~~**Suite de tests colgada**~~ — **resuelto el 2026-07-26**, ver sección 6. Queda pendiente sólo arreglar las 10 fallas viejas deseleccionadas en `ci.yml`. Detalle histórico:: quedan colgados los tests de billing/trial que devuelven 402 — ver sección 6. **Una de las dos causas ya está arreglada**: `conftest.py` no cerraba el engine async de SQLite (`aiosqlite` usa hilos no-daemon), así que correr un archivo suelto terminaba en verde pero el proceso nunca salía; se agregó `pytest_sessionfinish` con `dispose()`. Falta el cuelgue *dentro* del test. Camino sugerido: cambiar `pytest.ini` a `--timeout-method=signal` para que el timeout mate el test en vez de quedar colgado, arreglar la causa (todos tocan respuestas 402 con `TestClient`), y recién ahí ampliar `ci.yml` para que corra la suite entera en vez de 4 archivos elegidos a mano.
