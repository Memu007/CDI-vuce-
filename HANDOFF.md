...
- **SEGURIDAD CERRADA (2026-06-22):** Score 9/10. Fase 2 versión chica completada: blacklist jti + password policy en reset/change + deps transitivas actualizadas (aiohttp 3.14.1, Pillow 12.2.0, pyOpenSSL 26.3.0). 36 tests locales + 17 Railway + 3 prompt injection + verificación visual + 8 blacklist jti = todos PASS. Pendientes fase 2 grande: refresh token, migrar inline scripts (sacar unsafe-inline), test XSS end-to-end cuando Tarifar esté activo.
