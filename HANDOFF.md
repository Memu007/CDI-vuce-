...
- **Suite de tests colgada (prioridad después del deploy)**: tres tests de billing/trial dejan el proceso esperando para siempre — ver sección 6. Camino sugerido: cambiar `pytest.ini` a `--timeout-method=signal` para que el timeout mate el test en vez de quedar colgado, arreglar la causa (los tres tocan respuestas 402 con `TestClient`), y recién ahí ampliar `ci.yml` para que corra la suite entera en vez de 4 archivos elegidos a mano.
